...
PathDescriptionbackend/Node.js + Express APIs, controllers, middleware, and database modelsbackend/src/chatbot/Small Python (FastAPI) microservice for optional AI assistancefrontend/vite-project/React + Vite frontend (dev server usually on http://localhost:5173)
...
_(Best for development and debugging)_
...
You should receive JSON containing response, session\_id, and context.
...
| Browser CORS error       | Ensure [http://localhost:5173](http://localhost:5173) is allowed |
...
*   backend/.env exists and contains MONGODB\_URI and JWT\_SECRET
...
2.  The frontend calls **Express backend APIs**
    
3.  The backend routes requests to controllers which may:
...
4.  **WebRTC** establishes peer-to-peer audio/video between users
    
5.  Responses are returned to the frontend and the UI updates
...
Medora is built to be a **practical, developer-friendly telemedicine platform**, not just a demo.It emphasizes **real workflows**: scheduling, vitals, consultations, records, and coordination.
